...
class NLTKPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        """
        # X = X.str.replace(r"\b(?!3D|3d)\b\w*\d+\w+", "")
        return [
...
@timeit
def build_and_evaluate(X, y, classifier, stopwords, outpath=None, verbose=True):
    """
...
def construct(qna_file, isCsv=True):
    ...
    #Load data
    if isCsv:
        data = pd.read_csv(qna_file, encoding = "utf-8", header=None)
    else:
        data = qna_file
